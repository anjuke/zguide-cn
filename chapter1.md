...
* worker上游和任务分发器相连，下游和结果收集器相连，这就意味着你可以在不改动任务分发器和结果收集器的基础上添加任意多个worker进来。倘若worker是绑定至端点的，而非连接至端点，则每当添加新的worker时，我们除了需要准备更多的端点外，还要改动任务分发器和结果收集器。我们认为，任务分发器和结果收集器是这个网络结构中较为稳定的部分，因此应该由它们绑定至端点。而worker，是容易发生变更的部分，应该把它和端点解耦。

* 我们需要做一些同步的工作，等待worker全部启动之后再分发任务。这点在ZMQ中很重要，且不易解决。连接套接字的动作会耗费一定的时间。因此当有多个worker尝试连接任务分发器时，可能第一个成功连接的那个worker会在其他worker还在忙于建立连接的时间里收到一堆任务。也就是说，如果我们不进行同步，那这些任务根本就不会被并行地执行。尝试移除任务分发器中的getchar()等待，看看会发生什么。
